...
class Options {
	...
	public function register_settings(): void {
		...
		$this->register_section_event_post_created();

		// Add a settings section for the "Post Updated" event.
		$this->register_section_event_post_updated();
	}
	 ...
	 */
	public function register_section_event_post_created(): void {
		// Register the settings section.
		add_settings_section(
			'wp_hook_expose_event_post_created',
			__( 'Post Created Webhook', 'wp-hook-expose' ),
			array( $this, 'render_section_event_post_created' ),
			'wp_hook_expose'
		);

		// Register the settings field for the "Post Created" webhook URL.
		add_settings_field(
			'wp_hook_expose_event_post_created_url',
			__( 'Webhook URL', 'wp-hook-expose' ),
			array( $this, 'render_field_event_post_created_url' ),
			'wp_hook_expose',
			'wp_hook_expose_event_post_created'
		);
		...
		add_settings_field(
			'wp_hook_expose_event_post_created_body',
			__( 'Webhook Request Body', 'wp-hook-expose' ),
			array( $this, 'render_field_event_post_created_body' ),
			'wp_hook_expose',
			'wp_hook_expose_event_post_created'
		);
	}
	 ...
	 */
	public function register_section_event_post_updated(): void {
		// Register the settings section.
		add_settings_section(
			'wp_hook_expose_event_post_updated',
			__( 'Post Updated Webhook', 'wp-hook-expose' ),
			array( $this, 'render_section_event_post_updated' ),
			'wp_hook_expose'
		);

		// Register the settings field for the "Post Updated" webhook URL.
		add_settings_field(
			'wp_hook_expose_event_post_updated_url',
			__( 'Webhook URL', 'wp-hook-expose' ),
			array( $this, 'render_field_event_post_updated_url' ),
			'wp_hook_expose',
			'wp_hook_expose_event_post_updated'
		);
		...
		add_settings_field(
			'wp_hook_expose_event_post_updated_body',
			__( 'Webhook Request Body', 'wp-hook-expose' ),
			array( $this, 'render_field_event_post_updated_body' ),
			'wp_hook_expose',
			'wp_hook_expose_event_post_updated'
		);
	}
	 ...
	 */
	public function render_section_event_post_created(): void {
		?>
	 ...
	 */
	public function render_section_event_post_updated(): void {
		?>
	 ...
	 */
	public function render_field_event_post_created_url(): void {
		$options = get_option( 'wp_hook_expose' );
		?>
        <input type="text" name="wp_hook_expose[event_post_created][url]"
               value="<?php echo esc_attr( $options['event_post_created']['url'] ?? '' ); ?>">
	 ...
	 */
	public function render_field_event_post_created_body(): void {
		$options = get_option( 'wp_hook_expose' );
		?>
        <textarea name="wp_hook_expose[event_post_created][body]" rows="10"
                  cols="50"><?php echo esc_textarea( $options['event_post_created']['body'] ?? '' ); ?></textarea>
        <p class="description">
	 ...
	 */
	public function render_field_event_post_updated_url(): void {
		$options = get_option( 'wp_hook_expose' );
		?>
        <input type="text" name="wp_hook_expose[event_post_updated][url]"
               value="<?php echo esc_attr( $options['event_post_updated']['url'] ?? '' ); ?>">
        <p class="description"><?php esc_html_e( 'The URL to which the POST request will be sent.', 'wp-hook-expose' ); ?></p>
		<?php
	}
	 ...
	 */
	public function render_field_event_post_updated_body(): void {
		$options = get_option( 'wp_hook_expose' );
		?>
        <textarea name="wp_hook_expose[event_post_updated][body]" rows="10"
                  cols="50"><?php echo esc_textarea( $options['event_post_updated']['body'] ?? '' ); ?></textarea>
        <p class="description">
			<?php esc_html_e( 'The JSON body of the POST request.', 'wp-hook-expose' ); ?>
            <br>
			<?php esc_html_e( 'Will be merged with hook-specific data fields.', 'wp-hook-expose' ); ?>
